...
solve_sir <- function(beta, gamma, I0 = 1, n = n_wks, pop = N) {
...
prior_curves <- lapply(seq_len(n_prior), function(i) {
  ...
  I0_s     <- max(0.001, rlnorm(1, log(1), 2.0))
  rho_s    <- rbeta(1, 2, 10)
  lambda_s <- rexp(1, 0.5)
  beta_s   <- R0_s * gamma_s
  tryCatch({
    sol_s    <- solve_sir(beta_s, gamma_s, I0 = I0_s)
    sol_df   <- as.data.frame(sol_s)
    S_vec    <- sol_df$S
    inc      <- pmax(c(S_vec[1] - S_vec[2], -diff(S_vec[-1])), 1e-6)
    mu_s     <- rho_s * inc + lambda_s
...
p03 <- ggplot() +
  ...
  labs(title = "Prior Predictive Check",
       x = "Week", y = "Expected cases (rho * incidence + lambda)") +
  bw_theme
save_plot(p03, "03_prior_predictive.png")
cat("Plot 03 saved.\n")

# ── 3. Compile and fit Stan model ─────────────────────────────────────────────
...
cat(sprintf("\nPosterior medians: beta=%.4f  gamma=%.4f  rho=%.4f  I0=%.4f  lambda=%.4f\n",
...
sol_df <- as.data.frame(sol) %>%
  ...
  mutate(
    week      = time,
    S_lag     = lag(S, default = N - I0_med),
    incidence = pmax(S_lag - S, 1e-6),
    fitted    = rho_med * incidence + lambda_med
  ) %>%
  rename(Susceptible = S, Infectious = I, Recovered = R) %>%
  select(week, Susceptible, Infectious, Recovered, fitted)
...
p02 <- ggplot() +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~ . / 10, name = "Cases / fitted(t)")
  ) +
...
# ── Plot 04: Trace plots ──────────────────────────────────────────────────────
posterior_arr <- as.array(fit, pars = c("R0","gamma","rho","phi","I0","lambda"))
color_scheme_set("gray")

p04_trace <- mcmc_trace(posterior_arr,
                         pars = c("R0","gamma","rho","phi","I0","lambda"),
                         facet_args = list(nrow = 6, labeller = label_parsed)) +
  labs(title = "MCMC Trace Plots") +
  bw_theme
save_plot(p04_trace, "04_trace_plots.png", w = 9, h = 10)
cat("Plot 04 saved.\n")
...
h_I0     <- make_hist(I0_post,     expression(I[0]),      "I0")
h_lambda <- make_hist(lambda_post, expression(lambda),    "lambda")

p05 <- grid.arrange(h_beta, h_gamma, h_rho, h_phi, h_R0, h_I0, h_lambda,
                    nrow = 3, ncol = 3,
                    top = "Posterior Distributions (median + 95% CI)")
ggsave(file.path(plots_dir, "05_posterior_histograms.png"),
       plot = p05, width = 12, height = 9, dpi = 200, bg = "white")
cat("Plot 05 saved.\n")
...
p06 <- ggpairs(pair_df[idx, ],
  ...
  labs(title = "Joint Posterior Distributions") + bw_theme
...
pp_df <- data.frame(week = 1:n_wks,
                    med  = mu_med, lo50 = mu_lo50, hi50 = mu_hi50,
                    lo90 = mu_lo90, hi90 = mu_hi90, cases = cases)
...
p07 <- ggplot(pp_df, aes(x = week)) +
  ...
  labs(title = "Posterior Predictive Check",
       subtitle = "Dark: 50% CI | Light: 90% CI | Line: median",
       x = "Week", y = "Expected cases") + bw_theme
...
yrep_long <- as.data.frame(t(yrep_mat[draw_idx, ])) %>%
  mutate(week = 1:n_wks) %>%
  pivot_longer(-week, names_to = "draw", values_to = "y")
